...
- **Subscription quota isolation** — `quota discover --json` only checks ordinary local presence signals and never opens credentials or reaches the network. `quota fetch --product … --json` invokes only the requested providers, emits schema-versioned JSON on stdout, and isolates provider failures. Normalized quota snapshots may be cached under `~/.vibe-usage/quota-cache.json`; secrets are never cached, persisted by the CLI, uploaded, or mixed into parser/incremental state. Kimi reads its CLI-owned OAuth file without refreshing it; ZCode accepts only an explicitly supplied `Z_AI_API_KEY`. Cursor stays discovery-only unless an official/local stable quota protocol replaces cookie scraping.
