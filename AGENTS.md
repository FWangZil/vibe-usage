...
- One logical call can appear more than once (retry/copy). Dedupe by `message.id` (+ `requestId` when present), keeping the largest payload, so a zeroed copy never wins. Anonymous records with neither id stay per-file.
