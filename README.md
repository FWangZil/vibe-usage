...
| Devin (Windsurf) | `$XDG_DATA_HOME/devin/cli/sessions.db` (default `~/.local/share/devin/cli/`; Windows `%APPDATA%\devin\cli\`; legacy `cli_sessions.db` read alongside; fixture/relocation override: `VIBE_USAGE_DEVIN_DB`). Covers Devin CLI and Devin Desktop sessions alike (the app drives devin-cli over ACP). Real per-call token metrics from `message_nodes.chat_message` (`metadata.metrics`: input/output/cache-read/cache-creation, the last folded into input); per-message `generation_model` wins over `sessions.model` which can hold routing modes like `adaptive`. `/fork` and `/revert` duplicate turns across chains/sessions under the same `message_id`, so usage is deduplicated globally by `message_id` (highest-usage copy wins); abandoned-branch turns still count — the tokens were spent. Sessions soft-deleted via `/rm-session` (`hidden=1`) are excluded; internal summarize/compact prompts are not counted as user turns. The legacy Windsurf IDE's Cascade chats are encrypted on disk and Devin Cloud sessions are server-side only, so neither is covered |
